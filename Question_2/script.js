...
searchInput.addEventListener('change', ()=> console.log(inp))
...
fetch(
    ...
    .then((data) => {
        ...
        fetch('https://raw.githubusercontent.com/debojyoti/places-fake-rest-api/master/places.json').then(response => response.json()).then(placeData => {
            let cafePlaces = placeData.places;

            let mergedData = cafeNames.map(item => {
                let otherData = cafePlaces.find(element => element.id === item.location_id)
                return {...item, ...otherData}
            })


            // let filteredCafes = mergedData.filter(item => item['name'].includes(searchInput.value))
